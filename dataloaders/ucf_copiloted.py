...
class ClipSubstractMean(Normalize):
    def __call__(self, buffer):
        buffer = super().__call__(buffer)
        means = np.array((101.4, 97.7, 90.2))
        buffer -= means
        return buffer


class RandomCrop(object):
    def __init__(self, output_size=(112, 112), padding=10):
    ...
    def __call__(self, buffer):
        ...
        top = np.random.randint(0, h - new_h + 2 * self.padding)
        left = np.random.randint(0, w - new_w + 2 * self.padding)

        padded_buffer = np.pad(buffer, ((0, 0), (self.padding, self.padding), (self.padding, self.padding), (0, 0)), mode='constant')
        new_buffer = padded_buffer[:, top: top + new_h, left: left + new_w, :]
...
class CenterCrop(object):
    ...
    def __call__(self, buffer):
        ...
        top = int(round(h - new_h + 2 * self.padding) / 2.)
        left = int(round(w - new_w + 2 * self.padding) / 2.)
...
# Add these methods to your UCFDataset class

def remove_duplicates(self, buffer):
    # Implement a method to remove duplicate frames
    pass

def normalize(self, buffer):
    # Implement a method to normalize pixel values
    pass

def balance_classes(self):
    # Implement a method to balance classes
    pass

def quality_control(self):
    # Implement a method to check the quality of your data and labels
    pass

# Modify your __getitem__ method to include the new preprocessing steps
def __getitem__(self, index):
    # Loading and preprocessing.
    video_path = self.landmarks_frame.iloc[index, 0]
    # labels [0,100]
    ...
    buffer = self.remove_duplicates(buffer)
...
# Modify your DataLoader to include data splitting and loading optimization
train_data, test_data = train_test_split(UCFDataset(root_dir='./Dataset/UCF101_n_frames',